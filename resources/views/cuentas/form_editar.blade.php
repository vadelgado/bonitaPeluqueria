        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form class="needs-validation" method="POST" novalidate action="{{ url('/cuentas/actualizar/'.$ingreso->id_ingresos) }}">
                            @csrf
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label for="pago" class="col-md-6 col-form-label ">{{ __('Monto pagado') }}</label>
                                    <input id="pago" type="text" class="form-control @error('pago') is-invalid @enderror" name="pago"
                                           value="{{ old('pago', $ingreso->pago) }}" required autocomplete="pago" autofocus>
                                    @error('pago')
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <label for="personal" class="col-md-4 col-form-label ">{{ __('Personal') }}</label>
                                    <select name="personal" id="personal" required autocomplete="personal" class="form-select">
                                        ...
                                        @foreach ($listaPersonal as $personal)
                                            <option value="{{ $personal->identificacion }}"
                                                {{ old('personal', $ingreso->fk_identificacion) == $personal->identificacion ? 'selected' : '' }}>
                                    ...
                                    <select name="procedimiento" id="procedimiento" required autocomplete="procedimiento" class="form-select">
                                        ...
                                        @foreach ($listaServicios as $servicio)
                                            <option value="{{ $servicio->id_procedimiento }}"
                                                {{ old('procedimiento', $ingreso->fk_id_procedimiento) == $servicio->id_procedimiento ? 'selected' : '' }}>
